...
def entropy(data): 
    ...
    positive_ex = len(data.loc[data["PlayTennis"] == 'Y']) 

    negative_ex = len(data.loc[data["PlayTennis"] == 'N']) 
    ...
    return entropy #total entropy

 

def gain(s,data,attrib): #gain for specific attribute say, outlook,temperature,humidity

    values = set(data[attrib]) #different values for the attribute

    print(values) 

    gain = s #s is total entropy

    for val in values: #calculate gain for attribute by iterating each time considering a value
...
def get_attrib(data): #to get the attribute that provides best prediction(maximum gain value)
